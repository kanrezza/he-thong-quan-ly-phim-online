...
<head>
    ...
    <style>
        ...
        .form-grid {
            ...
            gap:20px; max-width:1040px;
        }
        .full { grid-column:1 / -1; }

        .card {
            border:1px solid rgba(255,255,255,0.08); border-radius:20px;
            padding:24px 26px; background:#0d1117;
        }
        ...
        .alert-success {
            ...
            padding:12px 16px; background:rgba(0,215,96,0.08); color:#bbf7d0; font-size:14px;
            margin-bottom:20px;
        }
        ...
        .ep-table input {
            padding:9px 11px; font-size:13px; border-radius:9px;
        }
        .ep-row td:first-child input { width:70px; }
        ...
        .btn-add-ep {
            ...
            cursor:pointer; margin-top:12px; transition:background 0.13s;
        }
        .btn-add-ep:hover { background:rgba(0,215,96,0.16); }

        .btn-del-ep {
            border:0; background:none; color:rgba(239,68,68,0.6); font-size:18px;
            cursor:pointer; line-height:1; padding:6px 4px;
        }
        ...
        <div class="form-grid">
            ...
            <div class="card full">
                ...
                <div style="display:grid; grid-template-columns:repeat(2,1fr); gap:16px">
                    <div style="grid-column:1/-1">
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" placeholder="Nhập tên phim"
                    ...
                    <div>
                        ...
                        <select name="type" id="typeSelect" required onchange="toggleEpisodes(this.value)">
                            <option value="Phim lẻ"    {{ old('type','Phim lẻ')==='Phim lẻ'   ?'selected':'' }}>Phim lẻ</option>
                            <option value="Phim bộ"    {{ old('type')==='Phim bộ'              ?'selected':'' }}>Phim bộ</option>
                            <option value="Chiếu rạp"  {{ old('type')==='Chiếu rạp'            ?'selected':'' }}>Chiếu rạp</option>
                        </select>
                    </div>
                    <div>
                        <label>Trạng thái</label>
                        <select name="status">
                            <option value="active"   {{ old('status','active')==='active'?'selected':'' }}>Hiện (active)</option>
                            <option value="inactive" {{ old('status')==='inactive'?'selected':'' }}>Ẩn (inactive)</option>
                        </select>
                    </div>
                    <div>
                        <label>Năm phát hành</label>
                        <input type="number" name="year" value="{{ old('year') }}" placeholder="{{ date('Y') }}" min="1900" max="2100">
                    </div>
                    <div>
                    ...
                    <div style="grid-column:1/-1">
                        ...
                        <textarea name="description" placeholder="Tóm tắt nội dung...">{{ old('description') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div style="display:flex; flex-direction:column; gap:14px">
                    <div>
                        <label>Poster phim (JPG/PNG, tối đa 4MB)</label>
                        ...
                        <img id="posterPreview" src="" alt="" style="display:none; margin-top:10px; max-width:100px; border-radius:8px; border:1px solid rgba(255,255,255,0.1)">
                    </div>
                    <div>
                        <label>Link video / YouTube URL <span style="color:#6b7280; font-weight:400">(cho Phim lẻ)</span></label>
            ...
            <div class="card full">
                ...
                <textarea name="actors" placeholder="VD: Ngô Kinh, Trương Dịch Hưng, Trương Mạn Ngọc..." style="min-height:80px">{{ old('actors') }}</textarea>
                <p style="color:#4b5563; font-size:12px; margin:6px 0 0">Nhập tên các diễn viên, cách nhau bởi dấu phẩy.</p>
            </div>

            {{-- Tập phim (chỉ hiện khi Phim bộ) --}}
            <div class="card full ep-section {{ old('type')==='Phim bộ' ? 'show' : '' }}" id="episodesSection">
            ...
            <div class="full" style="padding-top:4px">
                <button class="btn-save" type="submit">Lưu phim</button>
            </div>
...
function addEpisode() {
    ...
    tr.id = 'ep-' + n;
    tr.innerHTML = `
        <td><input type="number" name="episodes[${n}][episode_number]" value="${n}" min="1" style="width:64px"></td>
        <td><input type="text" name="episodes[${n}][title]" placeholder="Tên tập (tuỳ chọn)" style="width:100%"></td>
        <td><input type="text" name="episodes[${n}][video_url]" placeholder="Link video / YouTube" style="width:100%"></td>
        <td><input type="number" name="episodes[${n}][duration]" placeholder="Phút" min="1" style="width:90px"></td>
...
// Re-add old episodes if validation failed
@if(old('type') === 'Phim bộ' && old('episodes'))
    @foreach(old('episodes') as $i => $ep)
        (function(){
            epCount++;
            const n = epCount;
            const tbody = document.getElementById('epBody');
            ...
            tr.id = 'ep-' + n;
            tr.innerHTML = `
                <td><input type="number" name="episodes[${n}][episode_number]" value="{{ $ep['episode_number'] ?? $loop->iteration }}" min="1" style="width:64px"></td>
                <td><input type="text" name="episodes[${n}][title]" value="{{ $ep['title'] ?? '' }}" placeholder="Tên tập" style="width:100%"></td>
                <td><input type="text" name="episodes[${n}][video_url]" value="{{ $ep['video_url'] ?? '' }}" placeholder="Link video" style="width:100%"></td>
                <td><input type="number" name="episodes[${n}][duration]" value="{{ $ep['duration'] ?? '' }}" placeholder="Phút" min="1" style="width:90px"></td>
                <td><button type="button" class="btn-del-ep" onclick="removeEp(${n})">×</button></td>
            `;
            tbody.appendChild(tr);
        })();
