...
<head>
    ...
    <title>Lịch sử xem - Lumière</title>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        .page-header { display:flex; align-items:flex-end; justify-content:space-between; flex-wrap:wrap; gap:14px; margin-bottom:28px; }
        h1 { margin:0; font-size:30px; letter-spacing:-0.04em; }
        .subtitle { color:var(--muted); font-size:14px; margin:8px 0 0; }

        .btn-clear { border:1px solid rgba(239,68,68,0.35); border-radius:999px; padding:10px 18px; background:rgba(239,68,68,0.1); color:#fca5a5; font:inherit; font-size:13px; font-weight:900; cursor:pointer; }
        ...
        .movie-card { display:flex; flex-direction:column; overflow:hidden; border:1px solid var(--line); border-radius:18px; background:#0d1117; box-shadow:0 8px 30px rgba(0,0,0,0.28); transition:transform 0.18s, box-shadow 0.18s; }
        ...
        .card-body small { color:#bbf7d0; font-size:11px; font-weight:900; letter-spacing:0.1em; text-transform:uppercase; }
...
<body>
    <aside class="sidebar">
        ...
        <a class="side-item active" href="{{ route('history.index') }}">
    ...
    <main class="app">
        ...
        <div class="content">
            <div class="page-header">
                <div>
                    <h1>Lịch sử xem</h1>
                    <p class="subtitle">{{ $histories->total() }} phim đã xem</p>
                </div>
                @if($histories->total() > 0)
                    <form method="POST" action="{{ route('history.clear') }}">
                        @csrf
                        <button class="btn-clear" type="submit" onclick="return confirm('Xóa toàn bộ lịch sử?')">Xóa lịch sử</button>
                    </form>
                @endif
            </div>

            @if(session('history_msg'))
                <div class="alert alert-green">{{ session('history_msg') }}</div>
            @endif
            ...
            @else
                <div class="movie-row">
                    @foreach($histories as $h)
                        @if($h->movie)
                        <a class="movie-card" href="{{ route('movies.show', $h->movie) }}">
                            <div class="card-poster">
                                @if($h->movie->poster)
                                    <img src="{{ $h->movie->posterUrl() }}" alt="{{ $h->movie->title }}">
                                @else
                                    <div class="card-poster-placeholder"></div>
                                @endif
                            </div>
                            <div class="card-body">
                                <small>{{ $h->movie->category->name ?? '' }} · {{ $h->movie->type }}</small>
                                <h2>{{ $h->movie->title }}</h2>
                                <p>Xem lúc {{ $h->watched_at->format('H:i d/m/Y') }}</p>
                            </div>
                        </a>
                        @endif
                    @endforeach
                ...
                @if($histories->hasPages())
                    <div class="pagination">
                        @foreach($histories->links()->elements as $el)
                            ...
                            @endif
                        @endforeach
                        @if($histories->hasNextPage())<a href="{{ $histories->nextPageUrl() }}">›</a>@endif
                    </div>
                @endif
            @endif
        </div>
